...
import { type WeekBlockEvent, WeeklyBlockSchedule } from '../components/schedule/WeeklyBlockSchedule'
...
export default function Schedule({ loaderData }: Route.ComponentProps) {
  ...
  // Memoize events to prevent unnecessary re-renders of the schedule
  const events = useMemo<WeekBlockEvent[]>(
    () =>
      classes.map((c) => ({
        id: c.id,
  ...
  return (
    <div className="container relative mx-auto p-8 text-center">
      ...
      <div className="rounded-xl border border-[#ffd700]/20 bg-[#1a1a1a]/50 p-6 shadow-xl backdrop-blur-sm">
        <WeeklyBlockSchedule events={events} onEventClick={useCallback((event) => setSelectedEvent(event), [])} />
      </div>
